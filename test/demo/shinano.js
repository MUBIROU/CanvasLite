/***************************************************************************
 * shinano.js (ver.2017-10-24T13:30 build 145 RC2)
 ***************************************************************************/

if (shinano != window) { //名前空間を省略可能にするために
    var shinano = {}; //namescape(省略をしない前提であればconstにします)
}
...
shinano.Canvas =
    class Canvas extends shinano.AbstractCanvas { //Contener（委譲）を利用
        //static constant
        ...
        constructor(_id_or_width, _height = undefined) {
            ...
            this.__container = new shinano.Container(); //委譲
        ...
        addChild(_superDisplay) { //= addObserver()
            if (_superDisplay instanceof shinano.SpriteSheet) { //for SpriteSheet
        ...
        screenShot(_startX = 0, _startY = 0, _endX = undefined, _endY = undefined, _type = "jpeg") {
            ...
            return new shinano.Bitmap(_dataURL, _startX, _startY, _endX, _endY);
        }
...
shinano.SuperDisplay =
    class SuperDisplay { //super class
        ...
        hitTest(_target) { //Bitmap,Line,Rect,SpriteSheet,Videoに対応
            //for this
            if (!(this instanceof shinano.Circle)) { //thisがCircle以外（Bitmap,SpriteSheet）の場合…
            ...
            //for target
            if (!(_target instanceof shinano.Circle)) { //_targetがCircle以外（Bitmap,SpriteSheet）の場合…
                var _target_Top = _target.y;
                var _target_Left = _target.x;
                if (!(_target instanceof shinano.Rect)) {
...
shinano.Container =
    class Container extends shinano.SuperDisplay { //observer pattern
        constructor() {
            super();
...
shinano.Bitmap =
    class Bitmap extends shinano.SuperDisplay { //observer pattern
        // static constant
...
shinano.Circle =
    class Circle extends shinano.SuperDisplay { //observer pattern
...
shinano.Line =
    class Line extends shinano.SuperDisplay { //observer pattern
...
shinano.Rect =
    class Rect extends shinano.Line { //observer pattern
        constructor(_startX = 0, _startY = 0, _endX = 100, _endY = 100) {
            super();
...
shinano.SpriteSheet =
    class SpriteSheet extends shinano.Bitmap { //observer pattern
...
shinano.Text =
    class Text extends shinano.SuperDisplay { //observer pattern
        constructor(_text) {
            super();
...
shinano.Video =
    class Video extends shinano.SuperDisplay { //observer pattern

...
class Canvas extends toile.AbstractCanvas { //Contener（委譲）を利用
	...
	constructor(_id_or_width, _height=undefined) {
		...
		if (!("ontouchstart" in window)) { //for Desktop
	...
	reload() { //for Debugging
		this.__context2D.fillRect(
			-this.__canvas.width/2,
			-this.__canvas.height/2,
			this.__canvas.width,
	...
	__commonObserverAction(_observer, _theData, _containerAlpha=1) {
		...
		switch(_theData.type) {
			...
			case "Text":
				this.__context2D.font = _theData.size + "px " + _theData.font;
				this.__context2D.textBaseline = _theData.baseline;
				this.__context2D.textAlign = _theData.align;
				this.__context2D.fillStyle = _theData.fillStyle;
			...
			default: "ERROR: Canvas.__commonObserverAction()";
		}
	}
...
class Bitmap extends toile.SuperDisplay { //observer pattern
	...
	constructor(_path) {
		...
		//private variables (There are defaults)
		this.__isMouseDown = false; //mousedownしたか否か
		this.__isHitOutsideArea = false; //mousedown時に領域外でヒットしたか否か
		this.__isChoice = false; //MouseUpOutside用
		this.__scale = 1;
		this.__scaleX = 1;
		this.__scaleY = 1;
	...
	addEventListener(_event, _function, _isCircle=false) {
		switch (_event) {
			case "mousedown": this.__mouseDownHandler = _function; break;
			case "mouseup": this.__mouseUpHandler = _function; break;
			case "mouseupoutside": this.__mouseUpOutsideHandler = _function; break;
			case "load": this.__loadHandler = _function; break;
			default: throw new Error("Bitmap.addEventListener()");
		}
	...
	removeEventListener(_event) {
		switch (_event) {
			case "mousedown": this.__mouseDownHandler = undefined; break;
			case "mouseup": this.__mouseUpHandler = undefined; break;
			case "mouseupoutside": this.__mouseUpOutsideHandler = undefined; break;
			case "load": this.__loadHandler = undefined; break;
			default: throw new Error("Bitmap.removeEventListener()");
		}
	}
	...
	__load_image_method(_e) { //if .png or .jpg can be read...
		...
		this.__originHeight = this.__height = _e.currentTarget.height;
	...
	update() { //override
		...
		if (this.__isReloadPermitted) {
			if (this.__state == "play") {
				...
				if (this.__count != 0) {
					this.__currentframe = this.__count;
				} else {
...
class Sound {
	...
	removeEventListener(_event) {
		switch (_event) {
			case "ended": this.__endedHandler = undefined; break;
			default: throw new Error("Sound.removeEventListener()");
		}
	}
